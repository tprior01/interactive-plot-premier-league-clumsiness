...
DB = 'plerrors.db'
conn = sql.connect(DB)
totalsQuery = open('totalsquery.sql').read()
...
barStacks = ['redcards', 'errors', 'pensconceded', 'owngoals']
...
playerName = AutocompleteInput(title='Highlighted player name:', value='Henderson', completions=playerShortNames,
                               restrict=True, case_sensitive=False)
playerID = Select(title='Highlighted player ID:', value='3712', options=idMap[playerName.value])
...
q.vbar_stack(barStacks, x='seasons', width=0.2, color=barColours, source=seasonal, legend_label=categories)
q.y_range.start = 0
...
def updateBar():
    try:
        df = pd.read_sql(seasonalQuery, con=conn, index_col='cat', params={"xid": playerID.value})
        df = df.T
        df = df[df['minutes'] != 0]
        seasonal.data = dict(
            seasons=df.index.values,
            redcards=df['redcards'],
            pensconceded=df['pensconceded'],
            errors=df['errors'],
            owngoals=df['owngoals']
        )
...
def updateSize():
    ...
    p.title.text = f'{size} players selected'
